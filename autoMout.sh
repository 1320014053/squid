...
htpasswd -cb /etc/squid/passwd sbdjm sbdjm
htpasswd -b /etc/squid/passwd "$name1" "$name1"
htpasswd -b /etc/squid/passwd "$name2" "$name2"
htpasswd -b /etc/squid/passwd "$name3" "$name3"
echo "$name1" >> /etc/squid/uu11.list
echo "$name2" >> /etc/squid/uu22.list
echo "$name3" >> /etc/squid/uu33.list
rm -r /etc/squid/squid.conf
wget -O squid.conf -P /etc/squid/ https://raw.githubusercontent.com/1320014053/squid/master/squidMout.conf
